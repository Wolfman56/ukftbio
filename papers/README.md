# Papers

Theoretical papers produced by the ukftbio collaboration.
Source of truth for all papers is `uktf/papers/`. Files here mirror that directory.

## Published / Pre-print

| # | File | Status | Summary |
|---|------|--------|---------|
| 01 | `01_Biological_Choice_Fields_UKFT_v1.0.md` | Pre-pub hypothesis | Fungal spike trains as UKFT choice operators; entangled-cluster formalism; 7 falsifiable predictions; mitochondrial/CMS Borda isomorphism. Cross-domain mirror: `uktf/papers/41` |

## Planned

- `THE_ENTROPIC_ORIGIN_OF_LIFE.md` — UKFT derivation of Darwinian evolution
- `UKFT_BOHMIAN_EVOLUTION.md` — natural selection as least-action pruning in phylogenetic choice-space
- `THE_GOD_ATTRACTOR_IN_BIOLOGY.md` — consciousness as ultra-high-ρ biological attractor
